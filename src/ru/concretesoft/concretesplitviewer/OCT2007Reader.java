...
public class OCT2007Reader extends SplitReader{
    ...
    public OCT2007Reader(File file)  throws IOException, NotRightFormatException {
         ...
         */
        String firstLineOfPattern = "\\s\\d+\\s+\\d+.*(\\s+(\\d{1,2}:)?\\d\\d:\\d\\d)+\\s*$";
        String secondLineOfPattern = "^\\s*(\\s+(\\d{1,2}:)?\\d\\d:\\d\\d)+\\s*$";
        String thirdLineOfPattern = "^-*\\s*$";
        //Pattern athletePattern = Pattern.compile(lineOnePattern + "\\n" + lineTwoPattern + "\\n" + lineThreePattern + "\\n");
